...
define([
...
], function (m, _, s, Mixin, Environments) {
  describe('Environments Model', function () {
    ...
    it("should initialize the environments with state depending upon the checkedAgents", function () {
      var checkedAgents = [ { environments : function(){ return ['Test']}},{ environments : function(){ return ['Test', 'Dev']}}];
      Environments.init(checkedAgents);
      expect(Environments.list.length).toBe(2);
      expect(Environments.list[0].name()).toBe('Dev');
      expect(Environments.list[1].name()).toBe('Test');

      expect(Environments.list[0].checked()).toBe(false);
      expect(Environments.list[0].indeterminate()).toBe(true);
      expect(Environments.list[1].checked()).toBe(true);
      expect(Environments.list[1].indeterminate()).toBe(false);
    });
  });
});
