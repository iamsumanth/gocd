...
define([
...
], function (m, _, s, Mixin, Resources) {
  describe('Resources Model', function () {
    ...
    it("should initialize the resources with state depending upon the checkedAgents", function () {
      var checkedAgents = [ { resources : function(){ return ['Linux']}}, { resources : function(){ return ['Linux', 'Firefox']}}];
      Resources.init(checkedAgents);
      expect(Resources.list.length).toBe(2);
      expect(Resources.list[0].name()).toBe('Firefox');
      expect(Resources.list[1].name()).toBe('Linux');

      expect(Resources.list[0].checked()).toBe(false);
      expect(Resources.list[0].indeterminate()).toBe(true);
      expect(Resources.list[1].checked()).toBe(true);
      expect(Resources.list[1].indeterminate()).toBe(false);
    });
  });
});
