...
define(['mithril', 'lodash', 'string-plus',
  'models/model_mixins', 'lib/filesize',
  'helpers/mrequest'], function (m, _, s, Mixins, filesize, mrequest) {
  var Agents = function (data) {

    this.disableAgents = function (uuids, message) {
      var json = {
        uuids:              uuids,
      ...
      m.request({
        ...
        message('Disabled ' + uuids.length + ' agent(s)');
      }, function (error) {
        message(error.message);
      })
    };

    this.deleteAgents = function (uuids, message) {
      ...
      m.request({
        ...
        message('Deleted ' + uuids.length + ' agent(s)');
      }, function (error) {
        message(error.message);
      });
    };

    this.enableAgents = function (uuids, message) {
      var json = {
        uuids:              uuids,
      ...
      m.request({
        ...
        message('Enabled ' + uuids.length + ' agent(s)');
      }, function (error) {
        message(error.message);
      })
    };

    this.updateResources = function (uuids, add, remove, message) {
      var data = {
        uuids:      uuids,
        operations: {
      ...
      m.request({
        ...
        message('Resource(s) modified on ' + uuids.length + ' agent(s)');
      }, function (error) {
        message(error.message);
      })
    };

    this.updateEnvironments = function (uuids, add, remove, message) {
      var data = {
        uuids:      uuids,
      ...
      m.request({
        ...
        message('Environment(s) modified on ' + uuids.length + ' agent(s)');
      }, function (error) {
        message(error.message);
      })
    };
  ...
  Agents.all = function (url, callback) {
    return m.request({
      method:        "GET",
      url:           url,
      config:        mrequest.xhrConfig.v2,
      unwrapSuccess: function (data) {
        return new Agents.fromJSON(data['_embedded']['agents']);
      }
    });
  };
